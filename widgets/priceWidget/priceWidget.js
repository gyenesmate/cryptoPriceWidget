...
function getDataAttributes(container, defaultConfig) {
    return {
        ...
        refreshInterval: parseInt(container.dataset.refreshInterval) || defaultConfig.refreshInterval
    };
}
...
function initWidget(container, dataAttributes) {
    const widgetHTML = `
        ...
        <div class="widget-body">
            ...
            <p>Price: <span class="priceDisplay">Loading...</span></p>
...
document.addEventListener("DOMContentLoaded", async function() {
    ...
    widgetData.forEach(({container, dataAttributes}) => initWidget(container, dataAttributes));

    setInterval(() => {
        widgetData.forEach(async ({container, dataAttributes}) => {
            const price = await fetchApiData(dataAttributes.currency, dataAttributes.fiat);
